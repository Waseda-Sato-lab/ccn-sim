...
//////////////////////////////////////////
  ...
  //Ipv4InterfaceContainer i = ipv4.Assign (ndc1[6]);

  BulkSendHelper source ("ns3::TcpSocketFactory",
                         InetSocketAddress (Ipv4Address::GetAny (), 9999));
  // Set the amount of data to send in bytes.  Zero is unlimited.
  source.SetAttribute ("MaxBytes", UintegerValue (1024));
  ApplicationContainer sourceApps = source.Install (nodes_net1[0][1].Get (0));
  ...
  PacketSinkHelper sink ("ns3::TcpSocketFactory",
                         InetSocketAddress (Ipv4Address::GetAny (), 9999));
  ApplicationContainer sinkApps = sink.Install (nodes_net1[0][2].Get (0));
